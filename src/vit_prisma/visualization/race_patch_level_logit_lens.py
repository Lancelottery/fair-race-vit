...
def display_grid_on_image_with_heatmap(image, patch_dictionary, patch_size=32,
                                       layer_idx=-1,
                                       imagenet_class_to_emoji=RACE_EMOJI,
                                       emoji_font_size=10,
    ...
    for patch_index, patch_data in sorted(patch_dictionary.items()):
        ...
        if row >= grid_size_y or col >= grid_size_x:
            continue
        ...
        annotations.append(go.layout.Annotation(x=col * patch_size + patch_size / 2,
                                                ...
                                                text=emoji,
                                                showarrow=False,
                                                font=dict(size=emoji_font_size, color=f"rgba(0,0,0,{emoji_alpha})")))
...
def display_patch_logit_lens(patch_dictionary, width=1000, height=1200, emoji_size=10, return_graph=False, show_colorbar=True, labels=None):
    ...
    for i, patch in enumerate(patch_dictionary.values()):
        for j, items in enumerate(patch):  # Extract class index directly from the patch_dictionary
            class_index = items[2]
            emoji = RACE_EMOJI.get(class_index, "")  # Use class index for emoji lookup, default to empty if not found
